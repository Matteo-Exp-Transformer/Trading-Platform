...
const { mockGetUser, mockFrom, mockSingle, mockSelect, mockInsert, mockOrder, mockEq } =
  vi.hoisted(() => ({
    mockGetUser: vi.fn(),
    mockFrom: vi.fn(),
    mockSingle: vi.fn(),
...
import { createChat, addMessage, loadMessages } from './chatData.js';

beforeEach(() => {
  vi.clearAllMocks();
  mockGetUser.mockResolvedValue({ data: { user: { id: 'user-1' } }, error: null });

  // catena: from().insert().select().single()
