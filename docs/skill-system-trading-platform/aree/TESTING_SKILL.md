...
> Stack: **Vitest** ovunque (stessa toolchain del client). Client: `jsdom` + setup
> `client/src/test/setup.js`. Server: Node puro + setup `server/test/setup-env.js` (carica `.env.local`),
> `testTimeout: 30000` perché alcuni test toccano **Supabase live** (RLS). Nessun runner E2E per ora.
>
...
- **Niente merge senza test verdi.** Il gate è `npm run validate` (lint + tutti i test).
...
> **Attenzione ai test server:** `server/test/auth-rls.test.js`, `chat-rls.test.js`,
> `supabaseAdmin.test.js` colpiscono **Supabase reale**. Se `.env.local` non ha
> `SUPABASE_URL` / `SUPABASE_SERVICE_ROLE_KEY` / `SUPABASE_ANON_KEY`, falliscono con errori di rete/credenziali:
> non è una regressione del codice, è ambiente mancante. Sono più lenti (timeout 30s) di proposito.
...
| Server — isolamento/DB | `server/test/{auth-rls,chat-rls,supabaseAdmin}.test.js` | **RLS live** |
| Server — salute | `server/test/health.test.js` | unit |

> Ordine di grandezza attuale: **~137 test client + ~77 server**. Il numero **cresce** — non fidarti
> di questa cifra come oracolo: **esegui** e leggi l'esito reale.
...
1. **`npm run validate`** — gate automatico. Se è rosso, la revisione **si ferma** e riporti cosa fallisce.
